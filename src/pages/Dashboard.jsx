...
export default function Dashboard() {
  ...
  const balance = totalBalance(transactions, today);
  const catData = spendingByCategory(transactions, period, categories, today);
  const recent = recentTransactions(transactions, 5, today);
  ...
  const upcomingInPeriod = transactions.filter((t) => isUpcoming(t, today) && inPeriod(t, period)).length;
  const countedInPeriod = transactions.filter((t) => !isUpcoming(t, today) && inPeriod(t, period)).length;

  const confirmDelete = () => {
    deleteTransaction(toDelete.id);
    toast({ title: STRINGS.dashboard.toastDeleted, description: STRINGS.dashboard.toastDeletedBody });
    setToDelete(null);
  };
    ...
    return (
      <div>
        ...
        <div className="card-soft mt-6 p-10 text-center">
          ...
          <h2 className="mt-5 font-display text-2xl font-semibold">{STRINGS.dashboard.emptyTitle}</h2>
          ...
          <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <button onClick={() => { setEditing(null); setFormOpen(true); }} className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground transition hover:bg-primary/90">
              <Plus className="h-4 w-4" /> {STRINGS.dashboard.emptyAdd}
            </button>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label={STRINGS.dashboard.totalIncome} value={formatMoney(totals.income, currency)} tone="income" icon={TrendingUp} accent="hsl(152 40% 30%)" sublabel={periodName} />
        <StatCard label={STRINGS.dashboard.totalExpenses} value={formatMoney(totals.expense, currency)} tone="expense" icon={TrendingDown} accent="hsl(4 60% 48%)" sublabel={periodName} />
        <StatCard label={STRINGS.dashboard.netPeriod} value={formatMoney(totals.net, currency)} tone="balance" icon={Scale} sublabel={periodName} />
        <StatCard label={STRINGS.dashboard.totalBalance} value={formatMoney(balance.balance, currency)} icon={Wallet} accent="hsl(28 58% 52%)" sublabel={STRINGS.common.allTime} />
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-5">
        ...
        <div className="card-soft p-5 lg:col-span-3">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-lg font-semibold">{STRINGS.dashboard.spendingByCategory}</h2>
            <Link to="/reports" className="text-sm text-muted-foreground hover:text-primary">{STRINGS.dashboard.reportsLink} <ArrowRight className="inline h-3.5 w-3.5" /></Link>
          </div>
          <div className="mt-5">
            {catData.rows.length ? (
              ...
              <EmptyState
                compact
                icon={TrendingDown}
                title={STRINGS.dashboard.noExpensesTitle}
                description={upcomingInPeriod ? STRINGS.dashboard.noExpensesCounted(upcomingInPeriod) : STRINGS.dashboard.noExpensesEmpty}
              />
            )}
          </div>
        </div>
        ...
        <div className="card-soft p-5 lg:col-span-2">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-lg font-semibold">{STRINGS.dashboard.recent}</h2>
            <Link to="/transactions" className="text-sm text-muted-foreground hover:text-primary">{STRINGS.dashboard.viewAll} <ArrowRight className="inline h-3.5 w-3.5" /></Link>
          </div>
          <div className="mt-3 space-y-0.5">
            {recent.length ? (
              ...
              <EmptyState compact icon={Wallet} title={STRINGS.dashboard.noCountedTitle} description={STRINGS.dashboard.noCountedBody} />
            )}
          </div>
        </div>
      </div>
      ...
      {countedInPeriod === 0 && upcomingInPeriod > 0 && (
        <div className="mt-5 rounded-2xl border border-dashed border-border bg-secondary/40 px-5 py-4 text-sm text-muted-foreground">
          {STRINGS.dashboard.upcomingBanner(upcomingInPeriod)}
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        title={STRINGS.dashboard.deleteDialogTitle}
        destructive
...
function PageHeader({ periodName, onAdd }) {
  return (
    <div className="flex flex-wrap items-end justify-between gap-3">
      <div>
        <h1 className="font-display text-3xl font-semibold tracking-tight">{STRINGS.dashboard.title}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{STRINGS.dashboard.overview(periodName)}</p>
      </div>
      {onAdd && (
